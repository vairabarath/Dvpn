...
# Script to compile proto files for the DVPN client peer
...
if [ -d "$OUTPUT_DIR/Client_peer/pb" ]; then
    echo "Moving files from nested directory..."
    mv "$OUTPUT_DIR/Client_peer/pb"/*.pb.go "$OUTPUT_DIR/"
    rm -rf "$OUTPUT_DIR/Client_peer"
fi
